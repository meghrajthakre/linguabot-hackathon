...
router.get("/", authMiddleware, async (req, res, next) => {
  try {
    const bots = await Bot.find({ owner: req.user.id })
      .select("-contentChunks") // Exclude heavy chunks
      .sort({ createdAt: -1 });
    res.json(bots);
  } catch (err) {
    next(err);
...
router.put("/:botId", authMiddleware, async (req, res, next) => {
  try {
    ...
    const { website, faqs = [], pricing = [], docs = "" } = req.body;
    ...
    if (website && website !== bot.website) {
      const existingWebsite = bot.websiteSources.find(
        (ws) => ws.url === website
      );

      if (!existingWebsite) {
        bot.websiteSources.push({
          url: website,
    ...
    res.json({
      success: true,
      bot,
      message: website ? "Bot updated! Website scraping started..." : "Bot updated successfully!",
    });
  } catch (err) {
    next(err);
  }
