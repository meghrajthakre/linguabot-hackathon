...
router.post("/", publicAuth, async (req, res) => {
  ...
  try {
    const bot = req.bot; // from publicAuth middleware
    let { message, language } = req.body;

    console.log(`[Public] Received message: "${message}" with language: ${language}`);
    ...
    // ============================
    // BUILD SYSTEM PROMPT WITH USER LANGUAGE (if provided)
    // ============================
    const systemPrompt = buildSystemPrompt(bot, intent, language);
    console.log(`[Public] Building prompt for language: ${language || bot.language}`);
    ...
    try {
      ...
      console.log(`[Public] Response received in language: ${language}`);
    } catch (apiErr) {
    ...
    try {
      await Conversation.create({
        ...
        responseTimeMs,
        intent,
        language: language || bot.language, // store the language actually used
        metadata: {
          businessType: bot.businessType,
          messageLength: sanitizedMessage.length,
          responseLength: aiResponse.length,
          hasActionStep:
            /\b(click|visit|email|call|check|log|go to|contact|reach|schedule|book)\b/i.test(aiResponse),
    ...
    res.json({
      ...
      aiResponse,
      intent,
      responseLanguage: language || bot.language, // return the language actually used
      responseTimeMs,
